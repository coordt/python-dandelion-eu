...
class TestDatagem(TestDatagemBase):
    ...
    def test_select_multiple(self):
        for item in self.datagem.items.select("name", "population").where(
            ...
            self.assertEqual(item, dict(name="Trento", population={"2011": 114063}))
    ...
    def test_select_concat(self):
        ...
        for item in items.select("population").where(
            ...
            self.assertEqual(item, dict(population={"2011": 114063}))

    def test_select_empty(self):
        self.assertEqual(
            list(self.datagem.items.select()[:1]), list(self.datagem.items[:1])
        )
    ...
    def test_where(self):
        for item in self.datagem.items.where(parentNames__municipality="Ala"):
            self.assertEqual(item["parentNames"]["municipality"], "Ala")
    ...
    def test_where_greater_then_equal(self):
        ...
        self.assertTrue(all(x["population"]["2011"] >= 114063 for x in items))
        self.assertTrue(any(x["population"]["2011"] == 114063 for x in items))
    ...
    def test_where_greater_then(self):
        ...
        self.assertTrue(all(x["population"]["2011"] >= 114063 for x in items))
        self.assertFalse(any(x["population"]["2011"] == 114063 for x in items))
    ...
    def test_get_with_no_result(self):
        with self.assertRaises(DandelionException) as context:
            self.datagem.items.select("name").get(acheneID=self._achene("42"))
        self.assertEqual(context.exception.message, "The requested item does not exist")
    ...
    def test_pagination(self):
        ...
        self.assertEqual(
            ...
            len(items),
            len(frozenset(x["acheneID"] for x in items)),
        )
    ...
    def test_invalid_slice(self):
        ...
        for the_slice in [slice(-1), slice(0, -1), slice(-1, None)]:
            with self.assertRaises(TypeError) as context:
                list(self.datagem.items[the_slice])
            self.assertEqual(
                str(context.exception), "Negative indexes are not supported"
            )
        ...
        for the_slice in [slice(0, None, -1), slice(0, None, 0)]:
            with self.assertRaises(TypeError) as context:
                list(self.datagem.items[the_slice])
            self.assertEqual(
                str(context.exception), "Non-positive step is not supported"
            )
    ...
    def test_negative_index(self):
        ...
        self.assertEqual(str(context.exception), "Negative indexes are not supported")
...
class TestDeprecatedCode(TestDatagemBase):
    def test_objects(self):
        with warnings.catch_warnings(record=True) as warn:
            ...
            self.assertEqual(
                '"objects" is deprecated, use "items" instead', str(warn[0].message)
            )
