""" tests can be run from the root dir.

Add APP_ID and APP_KEY, or TOKEN to a .env file, or set them in your environment to run the tests.
...
class TestDefaultConfiguration(TestCase):
    def tearDown(self):
        ...
        for key in ["app_id", "app_key", "token"]:
            if key in default_config:
                del default_config[key]

    def test_can_set_app_id(self):
        default_config["app_id"] = os.environ.get("APP_ID", "foo")

        with self.assertRaises(DandelionException) as context:
            Datagem("administrative-regions")

        self.assertEqual(context.exception.message, 'Param "app_key" is required')

    def test_can_set_app_key(self):
        default_config["app_key"] = os.environ.get("APP_KEY", "foo")

        with self.assertRaises(DandelionException) as context:
            Datagem("administrative-regions")

        self.assertEqual(context.exception.message, 'Param "app_id" is required')

    def test_can_authenticate(self):
        with self.assertRaises(DandelionException) as context:
            Datagem("administrative-regions")
        self.assertEqual(context.exception.message, 'Param "token" is required')
        ...
        default_config["app_id"] = os.environ.get("APP_ID", "foo")
        default_config["app_key"] = os.environ.get("APP_KEY", "bar")
    ...
    def test_cannot_set_other_params(self):
        ...
        self.assertEqual(context.exception.message, "invalid config param: foo")
        ...
        class TestClass(BaseDandelionRequest):
            ...
            def _get_uri_tokens(self):
                return (
                    ...
                    else super(TestClass, self)._get_uri_tokens()
                )
